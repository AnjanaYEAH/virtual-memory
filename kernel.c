...
x86_64_pagetable *copy_pagetable(x86_64_pagetable *currentPt, pid_t owner );
x86_64_pagetable *kernelAlloc();
...
void kernel(const char* command) {
    ...
    virtual_memory_map(kernel_pagetable, 0, 0, PROC_START_ADDR, PTE_P | PTE_W, NULL);
    virtual_memory_map(kernel_pagetable, console, console, PAGESIZE, PTE_P | PTE_U| PTE_W, NULL);
...
for (int j = 0; j<NPAGES;++j)
    ...
    pageinfo[i].owner = g_uint;
    pageinfo[i].refcount = 1;
    memset((x86_64_pagetable*)PAGEADDRESS(i), 0, PAGESIZE);
...
x86_64_pagetable *copy_pagetable(x86_64_pagetable *currentPt, pid_t owner )
{
    // set global owner var
    g_uint = owner;
    
    x86_64_pagetable *new = kernelAlloc();
    // only do kernel pagss
    for (uintptr_t v = 0; v<PROC_START_ADDR; v = v+PAGESIZE)
    {
        vamapping map = virtual_memory_lookup(currentPt, v);
        virtual_memory_map(new, v, map.pa, PAGESIZE, map.perm, kernelAlloc);
    }
    return new;
}
...
void process_setup(pid_t pid, int program_number) {
    ...
    pageinfo[i].refcount = 1;
    log_printf("pid processes: %d\n", pid); 
    virtual_memory_map(processes[pid].p_pagetable, stack_page, PAGEADDRESS(i), PAGESIZE, PTE_P | PTE_W | PTE_U, kernelAlloc);
    log_printf("pid processes state: %d\n", pid); 
    processes[pid].p_state = P_RUNNABLE;

}

void fork(void){
        log_printf("called fork%d\n");
        int f = 0;
        int i = 0;
        for (i = 1; i < NPROC; i++) {
            log_printf("process: %d\n", i);
        ...
        for (int va =0; va < MEMSIZE_VIRTUAL; va+= PAGESIZE) {
            ...
            //     continue;
            // }
            vamapping data = virtual_memory_lookup(current->p_pagetable,va);
                     log_printf("perm: %d\n", data.perm);
                     log_printf("process: %d\n", i);
            if (((data.perm & (PTE_W|PTE_P|PTE_U)) == (PTE_W|PTE_P|PTE_U)) && pageinfo[data.pn].owner>0) {
                ...
                uintptr_t pa = PAGEADDRESS(p);
                memcpy(pa, data.pa, PAGESIZE);
                //virtual_memory_map(processes[i].p_pagetable, console, console, PAGESIZE, PTE_P | PTE_U| PTE_W, NULL);
                virtual_memory_map(processes[i].p_pagetable, va, pa, PAGESIZE, data.perm,kernelAlloc);
                if (pageinfo[data.pn].refcount > 0 && pageinfo[data.pn].owner>0) {
                    pageinfo[data.pn].refcount++;
                    pageinfo[data.pn].owner = current -> p_pid;
                }
            }
...
void exception(x86_64_registers* reg) {
    ...
    case INT_SYS_PAGE_ALLOC: {
        ...
        if (!found){
            ...
            current->p_registers.reg_rax = -1;
            if(r >= 0){
                virtual_memory_map(current->p_pagetable, va, pa, PAGESIZE, PTE_P | PTE_W | PTE_U, NULL);
